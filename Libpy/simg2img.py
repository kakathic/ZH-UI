...
# From http://forum.xda-developers.com/galaxy-s2/general/ref-unpacking-repacking-stock-rom-img-t1081239
# -> from https://android.googlesource.com/platform/system/extras/
# Modified to actually regenerate full ext4 file so content can be extracted
# Runs faster that sgs2toext4.jar / sgs2converter.jar or (it seems) ext4_unpacker (https://sourceforge.net/projects/androidicsjbext/)
# Alex131089
# Fill to be verified

from __future__ import print_function
import getopt, posixpath, signal, struct, sys, os
...
def main():
  ...
  for _ in ['']: # P:indent ..
    ...
    for i in range(1,total_chunks+1):
      ...
      if chunk_type == 0xCAC1:
        if data_sz != (chunk_sz * blk_sz):
          ...
          print("Raw data", end="")
          if expand:
            dst.write(FH.read(data_sz))
          else:
            FH.seek(data_sz, 1) # https://docs.python.org/3/tutorial/inputoutput.html
        ...
        if data_sz != 4:
          ...
          if expand:
            # P:FILL DST
            frm = dst.tell()
            while dst.tell() < frm+chunk_sz*blk_sz:
              dst.write(fill_bin)
            # EOFILL ?
    ...
    if expand:
      dst.truncate(offset*blk_sz) # P:Can be removed for smaller file size
    
    print("     %10u            %7u         End" % (FH.tell(), offset))
    
    sys.stdout = stdout # restore print
