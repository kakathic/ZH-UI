...
def GetRawImageFileHandle(filepath, image_size):
	...
	if magic != 0xEF53 :
		print('%s Tập tin hệ thống không phải ext4 .' % image_name)
		sys.exit(1)
	...
	if block_size != 4096 :
		print('%s block_size phải là 4096.' % image_name)
		sys.exit(1)
	...
	if total_bytes != image_size:
		print('%s lỗi kích cỡ phân vùng.' % image_name)
		sys.exit(1)
...
def Compute(args):
	...
	print('')
	print(' Xử lý dữ liệu hoàn thành, trong thời gian %d giây' % tTime)

	if args.sha1 == True:
		print('\n  Lấy sha1 checksums: ' + sha1sum.hexdigest())

	if args.compress == True:
		sTime = time.time()
		CompressDatFile(prefix + '.new.dat', prefix + '.new.dat.br')
		tTime = time.time() - sTime
		print('\n  Nén xong, mất %d giây' % tTime)
...
def main(args):
	if not os.path.isfile(args.image):
		print('raw_image_file không tồn tại: %s' % args.image)
		sys.exit(2)

	if not os.path.isdir(args.outdir):
		print('thư mục không tồn tại: %s' % args.outdir)
		sys.exit(2)
